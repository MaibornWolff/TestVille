package de.maibornwolff.ste.tesla.inputFileParsing.jiraXRAY.collectItems;


import de.maibornwolff.ste.tesla.domainModell.*;
import de.maibornwolff.ste.tesla.inputFileParsing.jiraXRAY.JiraXrayParser;
...
/**
 * This class handles all extracted items through the {@link JiraXrayParser}.
 ...
 */
@Deprecated // todo: move everything to Epic.filterAndAssociateTestCases(List<Item>)
public class ExportHandler{
    ...
    /**
     * this method start the state-machine {@link JiraXrayParser} and runs it on the input file {@link #exportFileName}.
    ...
    private List<Item> extractAllItems() throws Exception{
        ...
        SAXParserFactory.newInstance().newSAXParser().parse(new File(this.exportFileName), jiraXrayParser);
        return jiraXrayParser.getCollectedItems();
    }
    ...
    private void associateEpicsToTheirTestCases() {
        ...
        for (Epic epic: this.getAllEpics()) {
            ...
            alreadyAssociatedTests.addAll(epic.getAssociatedTestCasses());
        }
