...
import de.maibornwolff.ste.tesla.domainModell.Epic;
import de.maibornwolff.ste.tesla.domainModell.Item;
import de.maibornwolff.ste.tesla.domainModell.TestCase;
...
class ExportHandlerTest {
    ...
    List<DynamicTest> extractionTestForManyItems () throws Exception {
        ...
        String firstKey = allEpics.stream().filter(x->x.getKey().equals("BMWGMDET-1424"))
                .map(x -> x.getAssociatedTestCasses().iterator().next().getKey())
